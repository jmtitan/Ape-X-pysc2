    ...
    def sample_batch(self, batch_size):
        ...
        with self.lock:
            ...
            for global_idx, local_idx in zip(global_idxes.tolist(), local_idxes.tolist()):
                ...
                assert local_idx < len(self.act_buf[global_idx]), 'index is {} but size is {}'.format(local_idx, len(
                    self.act_buf[global_idx]))
...
class Learner:
    def __init__(self, buffer, env_name=config.env_name, lr=config.lr, eps=config.eps, num_act=config.num_actions, ckpt=config.ckpt):
    ...
    def stats(self, interval: int):
        ...
        print('buffer size (frame): {}'.format(ray.get(self.buffer.__len__.remote())))
        print("Sampling rate (frame/s): {}".format(
            (ray.get(self.buffer.__len__.remote()) - self.last_bufferlen) / interval))
        self.last_counter = self.counter
        self.loss = 0
        self.last_bufferlen = ray.get(self.buffer.__len__.remote())
        return self.done
        ...
        while True:
            ...
            if done or len(self.local_buffer) == self.max_episode_length:
                ...
                if (len(sequence) == config.sequence_len):
                    self.replay_buffer.add.remote(sequence)
